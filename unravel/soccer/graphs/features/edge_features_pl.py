...
def compute_edge_features_pl(adjacency_matrix, p3d, p2d, s, velocity, team, settings, feature_dict):
    ...
    feature_func_map = {
        ...
        'dist_matrix': {'func': lambda value: value, 'defaults': {'value': np.linalg.norm(p3d[:, None, :] - p3d[None, :, :], axis=-1)}},
        'dist_matrix_normed': {'func': lambda value, max_distance : np.where(nan_mask, 0, normalize_distance(value, max_distance)), 'defaults': {'value': np.linalg.norm(p3d[:, None, :] - p3d[None, :, :], axis=-1), 'max_distance': max_dist_to_player}},
        ...
        'speed_diff_matrix': {'func': lambda value: value, 'defaults': {'value': np.nan_to_num(s[None, :] - s[:, None])}},
        'speed_diff_matrix_normed': {'func': lambda s, team, ball_id, settings: np.where(nan_mask, 0, normalize_speed_differences_nfl(s, team, ball_id, settings)), 'defaults': {'s': np.nan_to_num(s[None, :] - s[:, None]), 'team': team, 'ball_id': Constant.BALL, 'settings': settings}},
        ...
        'angle_pos_matrix': {'func': lambda vect_to_player_matrix: np.where(nan_mask, 0, np.arctan2(vect_to_player_matrix[:, :, 1], vect_to_player_matrix[:, :, 0])), 'defaults': {'vect_to_player_matrix': vect_to_player_matrix}},
        'pos_cos_matrix': {'func': lambda angle_pos_matrix: np.where(nan_mask, 0, normalize_sincos(np.nan_to_num(np.cos(angle_pos_matrix)))), 'defaults': {'angle_pos_matrix': np.nan_to_num(np.arctan2(vect_to_player_matrix[:, :, 1], vect_to_player_matrix[:, :, 0]))}},
        'pos_sin_matrix': {'func': lambda angle_pos_matrix: np.where(nan_mask, 0, normalize_sincos(np.nan_to_num(np.sin(angle_pos_matrix)))), 'defaults': {'angle_pos_matrix': np.nan_to_num(np.arctan2(vect_to_player_matrix[:, :, 1], vect_to_player_matrix[:, :, 0]))}},
        ...
        'angle_vel_matrix': {'func': lambda angle_between, combined_matrix: np.where(nan_mask, 0, np.apply_along_axis(angle_between, 2, combined_matrix)), 'defaults': {'angle_between': angle_between, 'combined_matrix': combined_matrix}},
        'vel_cos_matrix': {'func': lambda angle_vel_matrix: normalize_sincos(np.nan_to_num(np.cos(angle_vel_matrix))), 'defaults': {'angle_vel_matrix': angle_vel_matrix}},
